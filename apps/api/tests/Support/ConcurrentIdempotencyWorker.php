...
if (!is_array($arguments) || 6 !== count($arguments)) {
    throw new RuntimeException('Usage: ConcurrentIdempotencyWorker.php <case> <key> <originalId> <barrier> <worker>');
}

[, $case, $key, $originalId, $barrier, $worker] = array_map(static function (mixed $value): string {
    if (!is_string($value)) {
...
if (!in_array($case, ['create', 'duplicate', 'refund', 'transfer'], true)) {
...
if (!$execution instanceof IdempotentExecution) {
    throw new RuntimeException('Idempotent execution service is unavailable.');
}
...
$result = $execution->execute(
    ...
    function () use ($case, $container, $originalId, $barrier): IdempotentResponse {
        ...
        match ($case) {
            'create' => (function () use ($container): void {
                $createTransaction = $container->get(CreateTransaction::class);
                if (!$createTransaction instanceof CreateTransaction) {
                    throw new RuntimeException('CreateTransaction service is unavailable.');
                }
                $createTransaction(new CreateTransactionInput(
            ...
            'duplicate' => (function () use ($container, $originalId): void {
                $duplicateTransaction = $container->get(DuplicateTransaction::class);
                if (!$duplicateTransaction instanceof DuplicateTransaction) {
                    throw new RuntimeException('DuplicateTransaction service is unavailable.');
                }
                $duplicateTransaction($originalId);
            })(),
            'refund' => (function () use ($container, $originalId): void {
                $createRefund = $container->get(CreateRefund::class);
                if (!$createRefund instanceof CreateRefund) {
                    throw new RuntimeException('CreateRefund service is unavailable.');
                }
                $createRefund($originalId, new CreateRefundInput(
                    '00000000-0000-7000-8000-0000000000d1', ['value' => '1.00', 'assetCode' => 'EUR'], '2026-03-14',
                    'Concurrent idempotent refund', null, null, null,
                ));
            })(),
            'transfer' => (function () use ($container): void {
                $createTransfer = $container->get(CreateTransfer::class);
                if (!$createTransfer instanceof CreateTransfer) {
                    throw new RuntimeException('CreateTransfer service is unavailable.');
                }
                $createTransfer(new CreateTransferInput(
