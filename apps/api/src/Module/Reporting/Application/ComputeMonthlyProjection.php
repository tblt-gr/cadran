...
/**
 * One month of the projection for an already resolved workspace and workspace-local day. The
 * annual report calls it once per open month; ReadMonthlyProjection resolves the caller first.
 */
final readonly class ComputeMonthlyProjection implements MonthlyProjector
{
    ...
    public function __invoke(WorkspaceScope $workspace, CalendarMonth $month, \DateTimeImmutable $today): MonthlyProjectionView
    {
        ...
        $window = MonthlyRecapWindow::of($month, $today);
        try {
        ...
        return new MonthlyProjectionView(
            ...
            $netWorth,
            $policy->reference(),
            self::budgetExpenseCategoryIds($categoryFacts, $categoryMetrics),
        );
    }

    /**
     * @param list<BudgetCategoryFact>       $categories
     * @param list<MonthlyRecapCategoryView> $published  the categories the projection publishes a cell for
     *
     * @return list<string>
     */
    private static function budgetExpenseCategoryIds(array $categories, array $published): array
    {
        $kept = array_flip(array_map(static fn (MonthlyRecapCategoryView $view): string => $view->id, $published));
        $ids = [];
        foreach ($categories as $category) {
            if (isset($kept[$category->id]) && $category->budgetIncluded && 'EXPENSE' === $category->type) {
                $ids[] = $category->id;
            }
        }
