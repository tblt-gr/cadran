...
final readonly class BudgetPayload
{
    ...
    public static function of(array $body, array $expectedFields): self
    {
        ...
        foreach ($body as $key => $value) {
            if (!is_string($key)) {
                throw new \UnexpectedValueException('A budget field name must be a string.');
            }
            $fields[$key] = $value;
        }
        ...
        if ($submitted !== $expectedFields) {
            throw new \UnexpectedValueException('A budget body carries exactly its declared fields.');
        }
