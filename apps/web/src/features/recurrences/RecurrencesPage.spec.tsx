import type { Account, RecurrenceCandidate } from '@cadran/api-client';
...
function renderPage() {
  return render(<QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}><RecurrencesPage /></QueryClientProvider>);
}
...
describe('RecurrencesPage', () => {
  ...
  it('explains that a no-candidate scan leaves the books untouched', async () => {
    ...
    api.listRecurrences.mockImplementation(() => success({ items: [], page: 1, perPage: 100, total: 0 }));
    api.listAccounts.mockImplementation(() => success({ items: [account], page: 1, perPage: 100, total: 1 }));
    renderPage();

    expect(await screen.findByRole('heading', { name: 'Aucune proposition à confirmer' })).toBeTruthy();
  ...
  it('opens a shared confirmation modal and submits exact server strings', async () => {
    api.detectRecurrenceCandidates.mockImplementation(() => success({ items: [candidate], partial: false }));
    api.listRecurrences.mockImplementation(() => success({ items: [], page: 1, perPage: 100, total: 0 }));
    api.listAccounts.mockImplementation(() => success({ items: [account], page: 1, perPage: 100, total: 1 }));
    api.confirmRecurrence.mockImplementation(({ body }) => success({ id: 'recurrence', ...body }, 201));
    renderPage();
