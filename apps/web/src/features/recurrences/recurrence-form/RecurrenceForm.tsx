...
export function RecurrenceForm({
  ...
  return (
    <form className={styles.form} noValidate onSubmit={submit}>
      ...
      {submitError ? <p className={styles.alert} role="alert">{submitError}</p> : null}
      <div className={styles.fields}>
        <label>
          <span>{t('recurrences.fields.label')}</span>
          <input aria-invalid={showErrors && errors.label ? true : undefined} autoFocus onChange={(event) => set('label', event.target.value)} value={values.label} />
          {showErrors && errors.label ? <small>{t('recurrences.validation.label')}</small> : null}
        </label>
        <label>
          <span>{t('recurrences.fields.account')}</span>
          <select aria-invalid={showErrors && errors.accountId ? true : undefined} disabled={editing} onChange={(event) => set('accountId', event.target.value)} value={values.accountId}>
          ...
          {showErrors && errors.accountId ? <small>{t('recurrences.validation.account')}</small> : null}
        </label>
        <label>
          <span>{t('recurrences.fields.counterparty')}</span>
          <input onChange={(event) => set('counterparty', event.target.value)} value={values.counterparty} />
        </label>
        <label>
          <span>{t('recurrences.fields.amount')}</span>
          <input aria-invalid={showErrors && errors.expectedAmount ? true : undefined} autoComplete="off" inputMode="decimal" onChange={(event) => set('expectedAmount', event.target.value)} spellCheck={false} value={values.expectedAmount} />
          {showErrors && errors.expectedAmount ? <small>{t('recurrences.validation.amount')}</small> : null}
        </label>
        <label>
          <span>{t('recurrences.fields.tolerance')}</span>
          <input aria-invalid={showErrors && errors.amountTolerance ? true : undefined} autoComplete="off" inputMode="decimal" onChange={(event) => set('amountTolerance', event.target.value)} spellCheck={false} value={values.amountTolerance} />
          {showErrors && errors.amountTolerance ? <small>{t('recurrences.validation.tolerance')}</small> : null}
        </label>
        <label>
          <span>{t('recurrences.fields.interval')}</span>
          <select onChange={(event) => set('intervalKind', event.target.value as RecurrenceIntervalKind)} value={values.intervalKind}>
            {INTERVALS.map((interval) => <option key={interval} value={interval}>{t(`recurrences.intervals.${interval}`)}</option>)}
          </select>
        </label>
        <label>
          <span>{t('recurrences.fields.day')}</span>
          <input aria-invalid={showErrors && errors.dayOfPeriod ? true : undefined} inputMode="numeric" max={values.intervalKind === 'WEEKLY' ? 7 : 31} min={1} onChange={(event) => set('dayOfPeriod', event.target.value)} type="number" value={values.dayOfPeriod} />
          {showErrors && errors.dayOfPeriod ? <small>{t('recurrences.validation.day')}</small> : null}
        </label>
        {!editing ? <label>
          <span>{t('recurrences.fields.firstExpectedOn')}</span>
          <input aria-invalid={showErrors && errors.firstExpectedOn ? true : undefined} onChange={(event) => set('firstExpectedOn', event.target.value)} type="date" value={values.firstExpectedOn} />
          {showErrors && errors.firstExpectedOn ? <small>{t('recurrences.validation.date')}</small> : null}
        </label> : null}
      </div>
      <div className={styles.actions}>
        <button className="primary-action" disabled={pending} type="submit">{t(pending ? 'recurrences.form.saving' : 'recurrences.form.save')}</button>
      </div>
    </form>
  );
}
